...
const server = http.createServer(async (req, res) => {
    ...
    const route = decodeURIComponent(pathname); //will be encoded to the normal one
    ...
    if (method === 'GET') {
        if (route === '/') {
            ...
            if (result.fileData) { // if there is smth in the file, otherwise is err
                send(res, result);
            }
            else {
        ...
        else if (route === '/remove') { // assining value to route
            const body = await getRequestPostBodyData(req);
            register.remove(body.value)
                .then(result => sendJson(res, result))
                .catch(error => sendJson(res, error));
        } else {
            sendError(res, "Resource not found", register.TYPES.ERROR) //code=404 as a default
        }
    }
    else {
